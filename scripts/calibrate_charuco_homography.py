...
def _extract_charuco_points(detection, intrinsics: CameraIntrinsics | None = None) -> tuple[np.ndarray, np.ndarray] | None:
...
def main() -> None:
    ...
    with CameraCapture(config.camera) as camera:
        for _ in range(args.max_frames):
            ...
            extracted = _extract_charuco_points(detection, intrinsics=intrinsics)
